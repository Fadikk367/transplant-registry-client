...
const AddRequestForm = ({closeModal}: AddRequestFormProps) => {
  const queryClient = useQueryClient();
  const {handleSubmit, control} = useForm<AddRequestValues>();

  const onValidData: SubmitHandler<AddRequestValues> = async (values) => {
    try {
      await addOrganRequest(values);
      await queryClient.invalidateQueries(['organ-requests']);
      closeModal();
    } catch (error) {
  ...
  const onInvalidData: SubmitErrorHandler<AddRequestValues> = (errors) => {
    console.log({errors});
  }
  ...
  return (
    <Card elevation={13} sx={{width: 400, padding: '20px', ...style}}>
      <Stack spacing={3} component="form" onSubmit={onSubmit}>
        <Typography variant="h5" gutterBottom>Register new request for organ</Typography>
        <Controller
          name="organ"
          control={control}
        ...
        <Controller
          ...
          render={({ field }) => (
            <FormControl fullWidth>
              <InputLabel id="priority-label">Patient priority</InputLabel>
              <Select {...field} label="Patient priority" labelId="priority-label" required>
                <MenuItem value={PatientPriority.Critical}>Critical</MenuItem>
                <MenuItem value={PatientPriority.Normal}>Normal</MenuItem>
                <MenuItem value={PatientPriority.Low}>Low</MenuItem>
              </Select>
            </FormControl>
          )}
        />
